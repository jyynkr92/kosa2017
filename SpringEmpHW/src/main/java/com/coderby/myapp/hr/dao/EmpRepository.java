...
public class EmpRepository implements IEmpRepository {
	...
	public int getEmpCount() {
		String sql = "select count(*) form employees";
		return jdbcTemplate.queryForObject(sql, Integer.class);
	}

	@Override
	public int getEmpCount(int deptid) {
		String sql = "select count(*) from employees where dpartment_id=?";
	...
	public EmpVO getEmpInfo(String empid) {
		String sql = "select employee_id, first_name, last_name, "
				...
				+"commission_pct, manager_id, department_+id "
				+"from emplyees where employee_id=?";
	...
	public void insertEmp(EmpVO emp) {
		String sql = "insert into employees (employee_id, first_name, "
				+"last_naem, email, phone_number, hire_date, job_id, "
...
//교수님이 추가해놓은 부분 
	...
	//Map은 키와 벨류값을가지는데 벨류값으로 객체를 받아와야한다??
	@Override
